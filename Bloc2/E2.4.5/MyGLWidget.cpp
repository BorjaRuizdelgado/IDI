...
void MyGLWidget::initializeGL ()
{
  ...
  modelo2.load("/home/borja/Desktop/Codi-Esquelet/models/legoman.obj");
...
void MyGLWidget::paintGL () 
{
  ...
  if(!modelo1){
      ...
      glBindVertexArray(VAO_Modelo2);
      modelTransform(4);
      glDrawArrays(GL_TRIANGLES, 0, modelo2.faces().size() * 3);
    }
...
void  MyGLWidget::modelTransform (int number)
{
    if( number == 0){
      ...
      glUniformMatrix4fv(transLoc, 1, GL_FALSE, &transform[0][0]);
        ...
        transform = glm::rotate(transform,(float)M_PI/2,glm::vec3(0,1,0));
        transform = glm::rotate(transform,rotation,glm::vec3(0,1,0));
        transform = glm::translate(transform,-centroBaseModelo2);
        glUniformMatrix4fv(transLoc, 1, GL_FALSE, &transform[0][0]);
...
void MyGLWidget::mouseMoveEvent(QMouseEvent *e)
{
  ...
  if (dx > dy){   // gir psi respecte eixY
    ...
    psiChange((int)(psi*180/(float)M_PI)%360);//simulamos que los diales no tengan límite hacia un lado
    ...
    thetaChange((int)(theta*180/(float)M_PI)%360);
  }
  update ();
...
void MyGLWidget::createBuffers () 
{
  ...
  //Cargamos segundo modelo
  glGenVertexArrays(1, &VAO_Modelo2);
  glBindVertexArray(VAO_Modelo2);

  glGenBuffers(1, &VBO_Modelo2);
  glBindBuffer(GL_ARRAY_BUFFER, VBO_Modelo2);
  glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat) * modelo2.faces().size() * 3 * 3, modelo2.VBO_vertices(), GL_STATIC_DRAW);

  // Activem l'atribut vertexLoc
  glVertexAttribPointer(vertexLoc, 3, GL_FLOAT, GL_FALSE, 0, 0);
  glEnableVertexAttribArray(vertexLoc);

  glGenBuffers(1, &VBO_Modelo2Color);
  glBindBuffer(GL_ARRAY_BUFFER, VBO_Modelo2Color);
  glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat) * modelo2.faces().size() * 3 * 3, modelo2.VBO_matdiff(), GL_STATIC_DRAW);
...
void MyGLWidget::calculaCajaModelo2(){
    //calculamos caja
    float xmin, xmax, ymin, ymax, zmin, zmax;

    xmin = xmax = modelo2.vertices()[0];
    ymin = ymax = modelo2.vertices()[1];
    zmin = zmax = modelo2.vertices()[2];
    for (unsigned int i = 3; i < modelo2.vertices().size(); i += 3)
    {
      if (modelo2.vertices()[i+0] < xmin)
        xmin = modelo2.vertices()[i+0];
      if (modelo2.vertices()[i+0] > xmax)
        xmax = modelo2.vertices()[i+0];
      if (modelo2.vertices()[i+1] < ymin)
        ymin = modelo2.vertices()[i+1];
      if (modelo2.vertices()[i+1] > ymax)
        ymax = modelo2.vertices()[i+1];
      if (modelo2.vertices()[i+2] < zmin)
        zmin = modelo2.vertices()[i+2];
      if (modelo2.vertices()[i+2] > zmax)
        zmax = modelo2.vertices()[i+2];
    }
    scale2 = 1/(ymax-ymin);
    centroBaseModelo2 = glm::vec3((xmax+xmin)/2, ymin, (zmax+zmin)/2);
 }
...
void MyGLWidget::cambiaModelo(bool cambio){
    modelo1 = cambio;
    update();
}

void MyGLWidget::changePsi(int cambio){
    makeCurrent();
    psi = cambio*(float)M_PI/180;
    viewTransform();
    update();
}

void MyGLWidget::changeTheta(int cambio){
    makeCurrent();
    theta = cambio*(float)M_PI/180;
    viewTransform();
    update();
}
