...
void MyGLWidget::initializeGL ()
{
  ...
  homer.load("./models/Patricio.obj");
...
void MyGLWidget::iniCamera(){
    ...
    calculaCajaModelo();
    ra = 1.0f;
    obs = glm::vec3(0,0,2*radioEsfera);
    vrp = glm::vec3(0,3,0);
    up = glm::vec3(0,1,0);
    float d = 0;
    for (int i = 0; i < 3; i += 1){
    ...
    d = sqrt(d);
    zn = (d - radioEsfera) / 2.0; //no es necesaerio que esea entre 2 lo hice por que no tuve en cuenta el suelo al hacer la caja
    zf = d + radioEsfera;
...
void MyGLWidget::calculaCajaModelo(){
    ...
    radioEsfera = glm::distance(Pmax, Pmin)/2;
}
