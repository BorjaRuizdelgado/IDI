...
void MyGLWidget::initializeGL ()
{
  ...
  iniEscena();
  iniCamera();
  posFocus = glm::vec3(0,0.5,0);//inicializamos la posición del foco
  focusCol = glm::vec3(1,1,1); //se ve de color negro puesto que solo el suelo solo refleja el azul y al ser amarillo el foco este solo irradia rojo y verde.
  glUniform3fv(focusColLoc,1,&focusCol[0]);
  glUniform3fv(focusLoc,1,&posFocus[0]);
  transformPatricio = glm::vec3(0,0,0);
  focus = 1;
  cull = false;
}
...
void MyGLWidget::createBuffersPatricio ()
{
  ...
  patr.load("/home/borja/Desktop/Codi-Esquelet/models/Patricio.obj");
...
void MyGLWidget::carregaShaders()
{
  ...
  focusLoc = glGetUniformLocation(program->programId(), "posFocus");//localizamos la variable del foco en el shader
...
void MyGLWidget::modelTransformPatricio ()
{
  ...
  TG = glm::translate(TG, transformPatricio);
...
void MyGLWidget::calculaCapsaModel ()
{
  ...
  escala = 0.3/(maxy-miny); //calculamos escala del patricio para que mida 0.3
...
void MyGLWidget::keyPressEvent(QKeyEvent* event) 
{
  ...
  switch (event->key()) {
    ...
    case Qt::Key_L:{
      posFocus.x += 0.1;
      glUniform3fv(focusLoc,1,&posFocus[0]);//enviamos la variable al shader
      break;
    }
    case Qt::Key_K:{
          posFocus.x -= 0.1;
   ...
   case Qt::Key_F: {
      ...
      glUniform1f(focusTypeLoc,focus);
      update();
      break;
   }
   case Qt::Key_Up: {
      makeCurrent();
      transformPatricio.z -= 0.05;
      posFocus.z -= 0.05;
      glUniform3fv(focusLoc,1,&posFocus[0]);
      update();
      break;
  ...
  case Qt::Key_V:{
    ...
    if(lightOpen){
        ...
        glUniform3fv(focusColLoc,1,&v[0]);
        lightOpen = true;
    }
    break;
    update();
